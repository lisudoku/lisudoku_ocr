...
fn crop_image(image: &Mat) -> Result<Mat, Box<dyn std::error::Error>> {
  ...
  let cropped_image_ref = core::Mat::roi(image, roi)?;

  let cropped_image = cropped_image_ref.clone_pointee();
...
fn parse_given_digits(
  ...
  eprintln!("Parsing big digits");
  let mut given_digits: Vec<FixedNumber> = vec![];
  for row in 0..9 {
    for col in 0..9 {
      eprintln!("Processing (big) {} {}", row, col);
...
fn parse_candidates(
  ...
  eprintln!("Parsing candidate digits");
  let mut candidates: Vec<CellCandidates> = vec![];
  for row in 0..9 {
    for col in 0..9 {
      eprintln!("Processing (small) {} {}", row, col);
      ...
      if ocr_result.0.is_none() && !ocr_result.1.is_empty() {
        ...
        eprintln!("{},{} => {:?}", row, col, cell_candidates);
...
fn process_cell(
  ...
  let square_ref = Mat::roi(image, rect)?;
  let square = square_ref.clone_pointee();
  ...
  if DEBUG {
    eprintln!("Found rectangles: {} large, {} small", large_rects.len(), small_rects.len());
  }
  ...
  if large_rects.len() > 1 {
    ...
    let mut candidates: Vec<u32> = small_rects.into_iter().filter_map(|rect| {
      let pencilmark_ref = Mat::roi(&square, rect).unwrap();
      let pencilmark = pencilmark_ref.clone_pointee();
    ...
    if candidates.iter().any(|&candidate| candidate < 1 || candidate > 9) ||
      ...
      eprintln!("Found invalid or duplicate candidates: {:?}", &candidates);
      return Ok((None, vec![]))
    }
...
fn find_contour_rects(contours: Vector::<Vector::<Point>>, image_max_size: f32) -> Vec<Rect> {
...
fn detect_digit_in_square_full(square: &Mat, big_digit: bool, cell_candidate_options: &HashSet<u32>) -> Result<Option<u32>, Box<dyn std::error::Error>> {
  ...
  if let Err(e) = res {
    eprintln!("Error detect_digit_in_square: {}", e);
    eprintln!("Retrying with use_bw=true");
...
#[allow(unused)]
pub fn show_image(image: &Mat) -> Result<(), Box<dyn std::error::Error>> {
...
#[allow(unused)]
pub fn display_lines(image: &Mat, horizontal: &Vec<Line>, vertical: &Vec<Line>) -> Result<(), Box<dyn std::error::Error>> {
